...
from models.spells.action_components.component_archer_tripleshot_01 import ComponentArcherTripleshot01


class ArcherTripleshot(Action):

	def __init__(self, toon: Toon, talent_level: int):
		super().__init__(toon, talent_level, 95 - (15 * talent_level))
	...
	def get_speed(self) -> int:
		return self.toon.get_attribute(Attribute.SPEED) - 3

	def _compose(self) -> list[ActionComponent]:
		return [
			ComponentArcherTripleshot01(self)
		]

	def _finalise(self, targets: list[str]) -> bool:
		if not 0 < len(targets) < 4:
			return False
